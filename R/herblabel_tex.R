herblabel_tex <- function(infile = NULL, spellcheck = FALSE, outfile = "herblabel.tex"){
    ...
    for(i in 1:nrow(herbdat000)){
        ...
        "\\begin{tabular}{@{}p{9.4cm}}",
        
        #### Title of the Herbarium
        paste("\\ \\ \\ \\ ", 
                herbdat$HERBARIUM," \\hfill ",gsub("_", "", as.character(herbdat$GLOBAL_UNIQUE_IDENTIFIER)),"", sep = ""),
        ...
        paste("\\centerline{",herbdat$FAMILY,"}\\vspace{ 2mm }", sep = ""),
        "\\large{",
        #### SPECIES INFO
        ...
        ifelse(is.na(herbdat$LAT_DEGREE), "", paste("$",
               ...
               herbdat$ELEVATION,"m \\vspace{ 1mm } \\\\",sep = "")),
        ...
        ifelse(is.na(herbdat$ADDITIONAL_COLLECTOR), 
            ...
            paste("\\textbf{",
                   ...
                   herbdat$COLLECTOR_NUMBER,"\\hfill " ,toupper(format(as.Date(herbdat$DATE_COLLECTED), format="%d %b %Y")),"}\\vspace{2mm}\\\\",sep = "")
            ), 
        
        ##### COLLECTION NOTES
        ifelse(is.na(herbdat$ATTRIBUTES), "", paste(ifelse(is.na(herbdat$REMARKS)&is.na(herbdat$PROJECT), 
               "", paste(as.character(herbdat$ATTRIBUTES),"\\\\" )),sep = "")),
        
        ##### HABITAT,VEGETATION DESCRIPTION AND ASSOCIATED SPECIES
        ifelse(is.na(herbdat$REMARKS), "", paste(ifelse(is.na(herbdat$PROJECT), 
               "", paste(as.character(herbdat$REMARKS), "\\\\")), sep = "")),
