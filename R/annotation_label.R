...
annotation_label <- function(dat = NULL, infile = NULL, spellcheck = TRUE, outfile = "Annotation_Labels.rtf") {
    ...
    if(spellcheck){
        ...
        herbdat000$GENUS[ind] <- paste("\\cf2\\i0 Name not valid, Check Spelling at {\\field{\\*\\fldinst{HYPERLINK \"http://www.theplantlist.org/\"}}{\\fldrslt{\\ul\\cf2 http://www.theplantlist.org/}}} or {\\field{\\*\\fldinst{HYPERLINK \"http://frps.eflora.cn/\"}}{\\fldrslt{\\ul\\cf2 http://frps.eflora.cn/}}} for:\\i  ", herbdat000$GENUS[ind], sep = "")
    ...
    for (i in 1:nrow(herbdat000)) {
        ...
        ifelse(is.na(herbdat$TYPE_STATUS)|is.null(herbdat$TYPE_STATUS)|herbdat$TYPE_STATUS == "", "", 
            paste("{\\pard\\keep\\keepn\\fi0\\li0\\brsp20\\sb200\\sa50\\fs20\\b ", toupper(as.character(herbdat$TYPE_STATUS)), "\\b0  of:\\par }", sep = "")), 
                
        ifelse(is.na(herbdat$INFRASPECIFIC_RANK)|herbdat$INFRASPECIFIC_RANK == "", 
            paste("{\\pard\\keep\\keepn\\fi-288\\li288", ifelse(is.na(herbdat$TYPE_STATUS)|herbdat$TYPE_STATUS == "", "\\sb200", "\\sb50"), "\\sa100\\fs20\\b\\i ",
        ...
        ifelse(is.na(herbdat$TYPE_REF)|herbdat$TYPE_REF == "", "",        
            paste("{\\pard\\keep\\keepn\\fi0\\li0\\brsp20\\sb50\\sa50\\fs18  " , ifelse(is.na(herbdat$TYPE_REF )|herbdat$TYPE_REF == "", "", herbdat$TYPE_REF), " \\par}")), 
        paste("{\\pard\\keep\\sb300", ifelse((is.na(herbdat$DET_NOTE)|herbdat$DET_NOTE == "")&(is.na(herbdat$PROJECT)|herbdat$PROJECT == ""), "\\sa200","\\sa30"), "\\keepn\\fi0\\li0\\tqr\\tx4850   Det.: ", 
        ...
        ifelse(is.na(herbdat$DET_NOTE)|herbdat$DET_NOTE == "", "", 
            paste("{\\pard\\keep\\sb20", ifelse(is.na(herbdat$PROJECT)|herbdat$PROJECT == "", "\\sa200", "\\sa20"), "\\keepn\\fi0\\li0\\fs18   Note:", as.character(herbdat$DET_NOTE), " \\par}")
            ), 
