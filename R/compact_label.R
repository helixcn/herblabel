    ...
    for(i in 1:nrow(herbdat000)){
        ...
        if(spellcheck){ ### = TRUE
            ...
            if(!Cap(as.character(temp.genus)) %in% Cap(as.character(pgenus$GENUS))){
                ...
                "\\highlight6\\i0  (Genus not accepted by The Plant List Website)\\highlight0 ", sep = "")
            }
            ...
            if(!Cap(as.character(temp.family)) %in% Cap(as.character(pgenus$FAMILY))){
                ...
                "\\highlight6  (Family not accepted by The Plant List Website)\\highlight0 ", sep = "")
            }
        ...
        ifelse(is.na(herbdat$ADDITIONAL_COLLECTOR), 
            paste("{\\pard\\keep\\keepn\\fi0\\li0\\sb80\\tqr\\tx4850\\qj ",
        ...
        ifelse(!is.na(herbdat$TYPE_STATUS), 
            paste("{\\pard\\keep\\sa40\\keepn\\fi0\\li0\\tqr\\tx4850\\qr ", herbdat$TYPE_STATUS,
                 ...
                 " \\qr0\\par }",sep = ""),
            paste("{\\pard\\keep\\sa40\\keepn\\fi0\\li0\\tqr\\tx4850\\qr Det.: ",
