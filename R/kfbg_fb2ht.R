...
kfbg_fb2ht  <- function(infile, outfile = "herblabel_temp.csv", 
                                ...
                                )
{
	...
	for(i in 1:nrow(herbdat000)){
        ...
        COLLECTOR                   [i] <- ifelse(is.null(herbdat$COLL_TEAM                ),NA,                         herbdat$COLL_TEAM                )
        ADDITIONAL_COLLECTOR        [i] <- ifelse(is.null(herbdat$ADDITIONAL_COLLECTOR     ),NA,                         herbdat$ADDITIONAL_COLLECTOR     )
        COLLECTOR_NUMBER            [i] <- ifelse(is.null(herbdat$COLL_NUMBER              ),NA,                         herbdat$COLL_NUMBER              )
        ...
        ATTRIBUTES                  [i] <- ifelse(is.null(herbdat$PLANT_DESCRIPTION        ),NA,                         herbdat$PLANT_DESCRIPTION        )
