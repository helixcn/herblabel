...
herbarium_label <- function(dat = NULL,
    ...
    if (any(is.na(herbdat000$COLLECTOR_NUMBER))) {
        stop(paste(
            ...
            paste(which(
                is.na(herbdat000$COLLECTOR_NUMBER)
            ) + 1, collapse = ", ")
        ))
    }
    if (any(is.na(herbdat000$DATE_COLLECTED))) {
        stop(paste(
            ...
            paste(which(
                is.na(herbdat000$DATE_COLLECTED)
            ) + 1, collapse = ", ")
        ))
    }
    ...
    if (any(is.na(herbdat000$GENUS))) {
        warning(paste(
            "\"GENUS\" must be provided for row: ",
            paste(which(is.na(
                herbdat000$GENUS
            )) + 1, collapse = ", ")
        ))
    ...
    if (any(is.na(herbdat000$STATE_PROVINCE))) {
        warning(paste(
            "\"STATE_PROVINCE\" not provided for row: ",
            paste(which(
                is.na(herbdat000$STATE_PROVINCE)
            ) + 1, collapse = ", ")
        ))
    }
    if (any(is.na(herbdat000$COUNTY))) {
        warning(paste(
            "\"COUNTY\" not provided for row: ",
            paste(which(is.na(
                herbdat000$COUNTY
            )) + 1, collapse = ", ")
        ))
    ...
    if (any(is.na(herbdat000$DATE_IDENTIFIED))) {
        warning(paste(
            ...
            paste(which(
                is.na(herbdat000$DATE_IDENTIFIED)
            ) + 1, collapse = ", ")
        ))
    }
    ...
    formatdate <- function(x) {
        if (!is.na(suppressWarnings(as.integer(x)))) {
            if (!grepl("^darwin", R.version$os)) {
                x <-
                    as.Date(as.integer(x), origin = "1899-12-30")   ### Default in MacOS
            } else {
                x <-
                    as.Date(as.integer(x), origin = "1904-01-01") ### Default in Windows
            }
        }
    ...
    REPLACE <- function(x) {
        ...
        if (endchar == ",") {
            ...
            res <-
                substr(bbb,
                       start = 1,
                       stop = ifelse(unlist(lapply(yyy, function(x) {
                           max(x) - 1
                       })) > 1,
                       unlist(lapply(yyy, function(x) {
                           max(x) - 1
                       })) , nchar(bbb)))
        } else {
            res <- bbb
    ...
    italic_latin <- function(x) {
        ...
        res.split3 <-
            gsub(",|\\.", "", res.split) ### will be compared with the Genera
        ...
        res.split[found] <-
            paste("\\cf3 \\i ", res.split[found], "\\i0\\cf0 ", sep = "")
    ...
    herbdat000$FAMILY                           <-
        toupper(herbdat000$FAMILY)
    herbdat000$GLOBAL_UNIQUE_IDENTIFIER         <-
        replace_space(herbdat000$GLOBAL_UNIQUE_IDENTIFIER)
    herbdat000$HERBARIUM                        <-
        replace_space(herbdat000$HERBARIUM)
    herbdat000$TITLE                            <-
        replace_space(herbdat000$TITLE)
    herbdat000$COLLECTOR                        <-
        replace_space(herbdat000$COLLECTOR)
    herbdat000$ADDITIONAL_COLLECTOR             <-
        replace_space(herbdat000$ADDITIONAL_COLLECTOR)
    herbdat000$COLLECTOR_NUMBER                 <-
        replace_space(herbdat000$COLLECTOR_NUMBER)
    herbdat000$DATE_COLLECTED                   <-
        replace_space(herbdat000$DATE_COLLECTED)
    herbdat000$LOCAL_NAME                       <-
        replace_space(herbdat000$LOCAL_NAME)
    herbdat000$FAMILY                           <-
        replace_space(herbdat000$FAMILY)
    herbdat000$GENUS                            <-
        replace_space(herbdat000$GENUS)
    herbdat000$SPECIES                          <-
        replace_space(herbdat000$SPECIES)
    herbdat000$AUTHOR_OF_SPECIES                <-
        replace_space(herbdat000$AUTHOR_OF_SPECIES)
    herbdat000$INFRASPECIFIC_RANK               <-
        replace_space(herbdat000$INFRASPECIFIC_RANK)
    herbdat000$INFRASPECIFIC_EPITHET            <-
        replace_space(herbdat000$INFRASPECIFIC_EPITHET)
    herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK     <-
        replace_space(herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK)
    herbdat000$COUNTRY                          <-
        replace_space(herbdat000$COUNTRY)
    herbdat000$STATE_PROVINCE                   <-
        replace_space(herbdat000$STATE_PROVINCE)
    herbdat000$COUNTY                           <-
        replace_space(herbdat000$COUNTY)
    herbdat000$LOCALITY                         <-
        replace_space(herbdat000$LOCALITY)
    herbdat000$IMAGE_URL                        <-
        replace_space(herbdat000$IMAGE_URL)
    herbdat000$RELATED_INFORMATION              <-
        replace_space(herbdat000$RELATED_INFORMATION)
    herbdat000$LAT_DEGREE                       <-
        replace_space(herbdat000$LAT_DEGREE)
    herbdat000$LAT_MINUTE                       <-
        replace_space(herbdat000$LAT_MINUTE)
    herbdat000$LAT_SECOND                       <-
        replace_space(herbdat000$LAT_SECOND)
    herbdat000$LAT_FLAG                         <-
        replace_space(herbdat000$LAT_FLAG)
    herbdat000$LON_DEGREE                       <-
        replace_space(herbdat000$LON_DEGREE)
    herbdat000$LON_MINUTE                       <-
        replace_space(herbdat000$LON_MINUTE)
    herbdat000$LON_SECOND                       <-
        replace_space(herbdat000$LON_SECOND)
    herbdat000$LON_FLAG                         <-
        replace_space(herbdat000$LON_FLAG)
    herbdat000$ELEVATION                        <-
        replace_space(herbdat000$ELEVATION)
    herbdat000$ATTRIBUTES                       <-
        replace_space(herbdat000$ATTRIBUTES) ### Change the first letter to Upper Case.
    herbdat000$REMARKS                          <-
        replace_space(herbdat000$REMARKS) ### Change the first letter to Upper Case.
    herbdat000$GEOREFERENCE_SOURCES             <-
        replace_space(herbdat000$GEOREFERENCE_SOURCES)
    herbdat000$PROJECT                          <-
        replace_space(herbdat000$PROJECT)
    herbdat000$IDENTIFIED_BY                    <-
        replace_space(herbdat000$IDENTIFIED_BY)
    herbdat000$DATE_IDENTIFIED                  <-
        replace_space(herbdat000$DATE_IDENTIFIED)
    herbdat000$TYPE_STATUS                      <-
        replace_space(herbdat000$TYPE_STATUS)
    herbdat000$PROCESSED_BY                     <-
        replace_space(herbdat000$PROCESSED_BY)
    herbdat000$DATE_LASTMODIFIED                <-
        replace_space(herbdat000$DATE_LASTMODIFIED)
    
    ####################
    ##### truncated the digits introduced by openxlsx for seconds
    
    herbdat000$LAT_DEGREE   <-
        as.character(as.integer(herbdat000$LAT_DEGREE))
    herbdat000$LAT_MINUTE   <-
        as.character(as.integer(herbdat000$LAT_MINUTE))
    ### truncated the the very small error introduced by openxlsx
    herbdat000$LAT_SECOND   <-
        as.character(round(as.numeric(herbdat000$LAT_SECOND), digits = 2))
    herbdat000$LON_DEGREE   <-
        as.character(as.integer(herbdat000$LON_DEGREE))
    herbdat000$LON_MINUTE   <-
        as.character(as.integer(herbdat000$LON_MINUTE))
    ### truncated the the very small error introduced by openxlsx
    herbdat000$LON_SECOND   <-
        as.character(round(as.numeric(herbdat000$LON_SECOND), digits = 2))
    ...
    if (spellcheck) {
        sptemp <-
            paste(
                ifelse(is.na(herbdat000$GENUS)                       , "",  herbdat000$GENUS),
                ifelse(is.na(herbdat000$SPECIES)                     , "",  herbdat000$SPECIES),
                ifelse(
                    is.na(herbdat000$AUTHOR_OF_SPECIES)           ,
                    "",
                    herbdat000$AUTHOR_OF_SPECIES
                ),
                ifelse(
                    is.na(herbdat000$INFRASPECIFIC_RANK)          ,
                    "",
                    herbdat000$INFRASPECIFIC_RANK
                ),
                ifelse(
                    is.na(herbdat000$INFRASPECIFIC_EPITHET)       ,
                    "",
                    herbdat000$INFRASPECIFIC_EPITHET
                ),
                ifelse(
                    is.na(herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK),
                    "",
                    herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK
                ),
                sep = " "
        ...
        tplsplist <-
            unique(
                c(
                    ...
                    herblabel::spfrps[, 2],
                    herblabel::spfoc[, 2],
                    herblabel::colcn2019[, 2]
                )
            ) ## automatically add the names for checking
        ...
        herbdat000$GENUS[ind] <-
            ifelse(is.na(herbdat000$GENUS[ind]),
                   "(Empty Species)",
                   herbdat000$GENUS[ind])
        
        herbdat000$GENUS[ind] <-
            paste(
                "\\cf3\\i0 This species can not be found in the embedded database in herblabel package. Check Spelling or Validity at {\\field{\\*\\fldinst{HYPERLINK \"http://www.theplantlist.org/\"}}{\\fldrslt{\\ul\\cf3 http://www.theplantlist.org/}}} or {\\field{\\*\\fldinst{HYPERLINK \"http://frps.eflora.cn/\"}}{\\fldrslt{\\ul\\cf3 http://frps.eflora.cn/}}} for:\\i  ",
                herbdat000$GENUS[ind],
                sep = ""
            )
        ...
        comment_genus[ind] <-
            paste(
                "This species can not be found in the embedded database in herblabel package",
                dat$GENUS[ind]
            )
        
        herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK[ind] <-
            paste(ifelse(
                is.na(herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK[ind]),
                "",
                herbdat000$AUTHOR_OF_INFRASPECIFIC_RANK[ind]
            ),
            "",
    ...
    for (i in 1:nrow(herbdat000)) {
        ...
        #### Check the Lat Flag
        if (any(
            !is.na(herbdat_temp$LAT_DEGREE),!is.na(herbdat_temp$LAT_MINUTE),!is.na(herbdat_temp$LAT_SECOND)
        ...
        #### Check the LON Flag
        if (any(
            !is.na(herbdat_temp$LON_DEGREE),!is.na(herbdat_temp$LON_MINUTE),!is.na(herbdat_temp$LON_SECOND)
    ...
    if (any(lat_check_ind_2)) {
        lat_check_ind_2_msg <-
            paste("LAT_FLAG not specified in row:",
    ...
    if (any(lon_check_ind_2)) {
        lon_check_ind_2_msg <-
            paste("LON_FLAG must be specified in row:",
    ...
    for (i in 1:nrow(herbdat000)) {
        ...
        if (spellcheck) {
            ...
            if (!Cap(as.character(temp.family)) %in% c(Cap(as.character(pgenus$FAMILY)), same_families)) {
                ### Consider the equal names for some families
                ...
                herbdat$FAMILY <-
                    paste(
                        "\\cf3 ",
                        as.character(temp.family),
                        "\\cf3 (Family not accepted at The Plant List Website.) ",
                        sep = ""
                    )
                comment_family[i] <-
                    paste(
                        gsub("\\\\\\cf3", "", as.character(temp.family)),
                        "Family not accepted at The Plant List Website."
                    )  ### For comments in the excel
            }
            ### check if the family provided in the excel matches APGIII or not
            fam.genus.temp <-
                data.frame(FAMILY = as.character(herbdat$FAMILY),
            ...
            if (any(as.character(Cap(fgmerge.temp$FAMILY.x)) !=  as.character(Cap(fgmerge.temp$FAMILY.y)) &
                ...
                if (unique(as.character(Cap(fgmerge.temp$FAMILY.x)))
                    ...
                    !grepl("could also be under",
                           as.character(temp.genus))) {
                    herbdat$FAMILY <-
                        paste("\\cf3 ", unique(as.character(
                            fgmerge.temp$FAMILY.x
                        )),
                        "  ", sep = "")
                    herbdat$GENUS  <-
                        paste(
                            "\\cf3 ",
                            unique(as.character(fgmerge.temp$GENUS)),
                            "\\i0\\cf3  (could also be under \"",
                            toupper(paste(
                                as.character(Cap(
                                    fgmerge.temp$FAMILY.y
                                ))[!as.character(Cap(fgmerge.temp$FAMILY.y)) %in% as.character(Cap(fgmerge.temp$FAMILY.x))],   collapse = "\", \""
                            )) ,
                        ...
                        if (!toupper(as.character(fgmerge.temp$FAMILY.x)) %in% toupper(same_families)) {
                            herbdat$GENUS <-
                                paste(
                                    "\\cf3 ",
                                    unique(
                                        as.character(fgmerge.temp$GENUS)
                                    ),
                                    "\\i0 \\cf3 (should be under \"",
                                    toupper(
                                        paste(
                                            as.character(fgmerge.temp$FAMILY.y),
                                            collapse = "\", \""
                                        )
                                    ) ,
                                    "\" according to The Plant List Website.) ",
            ...
            res <- c(
                    ...
                    paste(
                        ...
                        paste(
                            ...
                            ifelse(
                                ...
                                paste(
                                    "  ",
                                    as.character(herbdat$INFRASPECIFIC_RANK),
                                    sep = ""
                                )
                            ),
                            "\\i",
                            ...
                            ifelse(
                                ...
                                paste(
                                    "  ",
                                    as.character(herbdat$INFRASPECIFIC_EPITHET),
                                    sep = ""
                                )
                            ),
                            "\\i0",
                            ifelse(
                                ...
                                paste(
                                    "  ",
                                    as.character(herbdat$AUTHOR_OF_INFRASPECIFIC_RANK),
                                    sep = ""
                                )
                            ),
                            sep = ""
                ...
                paste(
                    ...
                    REPLACE(
                        paste(
                            toupper(ifelse(
                                is.na(herbdat$COUNTRY),
                                "",
                                herbdat$COUNTRY
                            )),
                            ", ",
                            ifelse(
                                is.na(herbdat$STATE_PROVINCE),
                                "",
                        ...
                        " \\.", "\\.", gsub("\\. \\.", "\\. ", gsub(
                            ...
                            REPLACE(
                                paste(
                                    ...
                                    "\\sb60 ",
                                    ifelse(
                                        is.na(herbdat$ATTRIBUTES),
                                        "",
                                        Cap2(as.character(herbdat$ATTRIBUTES))
                                    ),
                                    ifelse(is.na(herbdat$ATTRIBUTES), "", ". "),
                                    ifelse(is.na(herbdat$REMARKS)   , "", Cap2(
                                        as.character(herbdat$REMARKS)
                                    )),
                                    "\\sa80\\par}",
                                    sep = ""
                                )
                ...
                ifelse(
                    ...
                    paste(
                        ...
                        ifelse(
                            is.na(herbdat$TYPE_STATUS),
                            "",
                            paste(herbdat$TYPE_STATUS, " ", sep = "")
                        ),
                        ifelse(
                            is.na(herbdat$IDENTIFIED_BY),
                            "",
                            paste("Det.: ", herbdat$IDENTIFIED_BY, sep = "")
                        ),
        ...
        if (theme == "PE") {
            ...
            res <- c(
                #### Title of the Herbarium
                paste(
                    "{\\pard\\keep\\keepn\\fi0\\li0\\brsp20\\qc\\sb350",
                    ifelse(!is.na(herbdat$TITLE), "\\sa200", "\\sa400"),
                    "\\fs",
                    trunc(font_size * 20),
                    "\\ ",
                    herbdat$HERBARIUM,
                ...
                ##### COLLECTOR and COLLECTION NUMBER !
                paste(
                    "{\\pard\\keep\\keepn\\fi0\\sb100\\sa100\\fs",
                    trunc(font_size * 18),
                    "\\tqr\\b Collector: \\b0 ",
                    paste(herbdat$COLLECTOR, sep = ""),
                    "\\par}",
                    sep = ""
                ),
                paste(
                ...
                ##### COUNTY and LOCALITY
                paste(
                    "{\\pard\\keep\\keepn\\fi0\\li0\\sb100\\sa100\\fs",
                    trunc(font_size * 18),
                    " \\b Location: \\b0",
                    REPLACE(
                        paste(
                ...
                ifelse(
                    is.na(herbdat$FAMILY),
                    paste(
                        "{\\pard\\keep\\keepn\\fi0\\li0\\tx3000\\sb100\\sa100\\fs",
                        trunc(font_size * 18),
                        "\\b Family: ",
                        "\\b0 ",
                        "\\tab\\b Local Name: \\b0     ",
                        ifelse(
                            is.na(herbdat$LOCAL_NAME),
                            "",
                            herbdat$LOCAL_NAME
                        ),
                        "\\par }",
                ...
                ifelse(
                    ...
                    "",
                    paste(
                        "{\\pard\\keep\\keepn\\fi-288\\li288\\sb100\\sa100\\fs",
                        trunc(font_size * 18),
                        "\\b Species: \\b0\\i ",
                        REPLACE(
                            paste(
        ...
        if (theme == "KUN") {
            res <- c(
                ...
                ifelse(
                    ...
                    "",
                    paste(
                        "{\\pard\\keep\\keepn\\fi-288\\li288\\fs",
                        trunc(font_size * 18),
                        "\\b Species: \\b0\\i ",
                        REPLACE(
                            paste(
        ...
        if (theme == "HU") {
            res <- c(
                paste(
                    ...
                    paste(
                        ...
                        REPLACE(paste(
                            ifelse(
                                is.na(herbdat$STATE_PROVINCE),
                                "",
                                paste(as.character(herbdat$STATE_PROVINCE), ", ", sep = "")
                            ),
                            ifelse(
                                is.na(herbdat$COUNTY),
                                "",
                                paste(as.character(herbdat$COUNTY), ", "       , sep = "")
                            ),
                            ifelse(
                                is.na(herbdat$LOCALITY),
                                "",
                                paste(as.character(herbdat$LOCALITY)           , sep = "")
                            ),
                            sep = ""
                        )),
                        ". ",
                    ...
                    paste(
                        "{\\pard\\keep\\keepn\\sa100",
                        paste("\\fs", trunc(font_size * 18), sep = ""),
                        sep = ""
                    ),
                    " \\par }",
                    paste(
                        "{\\pard\\keep\\keepn\\fi0\\li0\\brsp20\\qc\\sb500\\sa100\\fs",
                        trunc(font_size * 20),
                        sep = ""
                    ),
                    "\\b ",
        ...
        NEW_DATE_COLLECTED[i] <-
            tryCatch(
                formatdate2(herbdat$DATE_COLLECTED),
                error = function(e) {
                    herbdat$DATE_COLLECTED
                }
            )
        NEW_DATE_IDENTIFIED[i] <-
            tryCatch(
                formatdate2(herbdat$DATE_IDENTIFIED),
                error = function(e) {
                    herbdat$DATE_IDENTIFIED
                }
            )
        temp2 <-
            c(temp2, res)         ### Add label to the RTF file.
    ...
    res <-
        template[!template %in% ""] ## Omit the rows without any information
    res <-
        res[!res %in% " "]          ### Omit the rows without any information
    res <-
        replace_space(res)          ### replace the space at the beginning or ending.
    ...
    return(invisible(list(
        dat = dat, modified_dat = modified_dat
    )))
}
