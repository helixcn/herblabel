...
function(dat = NULL){
    ...
    if(!"NUMBER_OF_LABELS_TO_CREATE" %in% col_names){
        ...
        NUMBER_OF_LABELS_TO_CREATE <- as.character(dat$NUMBER_OF_LABELS_TO_CREATE)
    }
    ### If any is missing, replace with ONE
    NUMBER_OF_LABELS_TO_CREATE[is.na(NUMBER_OF_LABELS_TO_CREATE)] <- 1
    
    ### Convert to integers
    NUMBER_OF_LABELS_TO_CREATE <- as.integer(NUMBER_OF_LABELS_TO_CREATE)
    
    ### All the integers < 1 will be replaced using one to avoid typo error.
    if(any(which(NUMBER_OF_LABELS_TO_CREATE < 1))) {
        warning(paste("In Column NUMBER_OF_LABELS_TO_CREATE, value in row:",
                      paste(which(NUMBER_OF_LABELS_TO_CREATE < 1), collapse = ","),
                      " less than one. Replaced using one.",sep = ""))
        NUMBER_OF_LABELS_TO_CREATE[NUMBER_OF_LABELS_TO_CREATE < 1] <- 1
    }
    
    if(any(which(is.na(NUMBER_OF_LABELS_TO_CREATE)))){
        warning(paste("In Column NUMBER_OF_LABELS_TO_CREATE check rows:",
                      paste(which(is.na(NUMBER_OF_LABELS_TO_CREATE)),
                            collapse = ","),sep = ""))
        NUMBER_OF_LABELS_TO_CREATE[is.na(NUMBER_OF_LABELS_TO_CREATE)] <- 1
    }
