...
kfbg_existing_db2ht  <- function(infile, outfile = "herblabel_temp.csv", 
                                ...
                                )
{
	...
	for(i in 1:nrow(herbdat000)){
        ...
        DATE_COLLECTED              [i] <- ifelse(is.null(herbdat$DATE_OF_COLLECTION                        ),NA,                         herbdat$DATE_OF_COLLECTION                        )
        LOCAL_NAME                  [i] <- ifelse(is.null(herbdat$ACCEPTED_CHINESE_NAME                     ),NA,                         herbdat$ACCEPTED_CHINESE_NAME                     )
        FAMILY                      [i] <- ifelse(is.null(herbdat$FAMILY                                    ),NA,                         herbdat$FAMILY                                    )
        GENUS                       [i] <- ifelse(is.null(herbdat$NOMEN_GENERARUM                           ),NA,                         herbdat$NOMEN_GENERARUM                           )
        SPECIES                     [i] <- ifelse(is.null(herbdat$SCIENTIFIC_NAME                           ),NA,                         herbdat$SCIENTIFIC_NAME                           )
        AUTHOR_OF_SPECIES           [i] <- ifelse(is.null(herbdat$SCIENTIFIC_AUTHORITY_FOR_SPECIES          ),NA,                         herbdat$SCIENTIFIC_AUTHORITY_FOR_SPECIES          )
        INFRASPECIFIC_RANK          [i] <- ifelse(is.null(herbdat$FIELD_ID_INFRA_RANK                       ),NA,                         herbdat$FIELD_ID_INFRA_RANK                       )
        INFRASPECIFIC_EPITHET       [i] <- ifelse(is.null(herbdat$SPECIES_OR_VARIETY_OR_CULTIVATED_NAME     ),NA,                         herbdat$SPECIES_OR_VARIETY_OR_CULTIVATED_NAME     )
        AUTHOR_OF_INFRASPECIFIC_RANK[i] <- ifelse(is.null(herbdat$SCIENTIFIC_AUTHORITY_FOR_VARIETY          ),NA,                         herbdat$SCIENTIFIC_AUTHORITY_FOR_VARIETY          )
        ...
        LOCALITY                    [i] <- ifelse(is.null(herbdat$LOCATION_OF_COLLECTION_IN_ENGLISH         ),NA,                         herbdat$LOCATION_OF_COLLECTION_IN_ENGLISH         )
        IMAGE_URL                   [i] <- ifelse(is.null(herbdat$LINK_OF_PHOTO                             ),NA,                         herbdat$LINK_OF_PHOTO                             )
        RELATED_INFORMATION         [i] <- ifelse(is.null(herbdat$RELATED_INFORMATION                       ),NA,                         herbdat$RELATED_INFORMATION                       )
        LAT_DEGREE                  [i] <- ifelse(is.null(herbdat$LATITUDE                                  ),NA,                         herbdat$LATITUDE                                  )
        ...
        LON_DEGREE                  [i] <- ifelse(is.null(herbdat$LONGITUDE                                 ),NA,                         herbdat$LONGITUDE                                 )
        ...
        ATTRIBUTES                  [i] <- ifelse(is.null(herbdat$FIELD_NOTES                               ),NA,                         herbdat$FIELD_NOTES                               )
        ...
        DATE_LASTMODIFIED           [i] <- ifelse(is.null(herbdat$DATE_OF_DATA_ENTRY                        ),DATE_LASTMODIFIED_ARGUMENT, herbdat$DATE_OF_DATA_ENTRY                        )
